// ============================================================
// המדריך ליזמי נדל"ן צעירים — מחולל הכתבה השבועית.
// קורא ל-Claude עם mentor_guide/prompt.md, מקבל MDX, וכותב קובץ
// מודול תחת src/content/mentorguide/<date>-<slug>.js — המבנה שטוען
// src/lib/mentorguide.js אוטומטית.
//
// הרצה: ANTHROPIC_API_KEY=... node mentor_guide/scripts/generate-article.mjs
// ============================================================
...
const contentDir = join(root, 'src', 'content', 'mentorguide')
...
const obj = {
  ...
  title: fm.title || '',
  date,
  author: fm.author || 'המערכת · ליווי יזמי נדל"ן',
  authorTitle: fm.authorTitle || 'המדריך ליזמי נדל"ן צעירים · קבוצת קורקוס',
  category: fm.category || 'איתור ובדיקה',
  tags,
...
const file = `// נוצר אוטומטית — המדריך ליזמי נדל"ן צעירים · ${date}\nexport default ${JSON.stringify(obj, null, 2)}\n`
