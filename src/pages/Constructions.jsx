...
/* טור ביצוע ובנייה ("המדריך לתהליך הבנייה") — עמוד רשימה. אותו עיצוב כמו טור היזמות. */
export default function Constructions() {
  const { lang } = useI18n()
  const all = useConstructions()
  ...
  return (
    <>
      <PageHeader
        eyebrow="טור ביצוע ובנייה"
        title="המדריך לתהליך הבנייה"
        lead="פרשנות שבועית, מקצועית ועניינית מהשטח: שלבי ביצוע, בטיחות, איכות וליקויים, רישוי וניהול אתר. כל מה שחשוב לדעת בתהליך הבנייה, מאת מערכת קבוצת קורקוס."
        crumbs={[{ label: 'ביצוע', to: '/divisions/execution' }, { label: 'הטור' }]}
      />

      <section className="section yz-page">
        <div className="container">
          {all.length === 0 ? (
            <p className="yz-empty">בקרוב, כתבות חדשות במדריך לתהליך הבנייה.</p>
          ) : (
            <>
              {cats.length > 1 && (
