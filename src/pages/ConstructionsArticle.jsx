...
import { useConstruction, useConstructions } from '../lib/constructions.js'
...
export default function ConstructionsArticle() {
  ...
  const { lang } = useI18n()
  const article = useConstruction(slug)
  const allArticles = useConstructions()
  ...
  const jsonLd = {
    ...
    mainEntityOfPage: { '@type': 'WebPage', '@id': `${SITE}/constructions/${article.slug}` },
  ...
  return (
    <article className="yz-article">
      <Seo
        title={article.metaTitle || `${article.title} · המדריך לתהליך הבנייה`}
        description={article.metaDescription || article.excerpt}
        image={coverUrl}
        jsonLd={jsonLd}
      />
        ...
        <Reveal className="container yz-body">
          ...
          <div className="yz-author-card">
            <div className="yz-author-card__txt">
              <strong>צריכים ליווי ביצוע מקצועי?</strong>
              <span>צוות הביצוע של קבוצת קורקוס מלווה פרויקטים מהיסודות ועד המסירה.</span>
            </div>
            <Link to="/divisions/execution" className="btn btn--dark">
              לשירותי הביצוע <Icon name="arrowLeft" size={18} />
            </Link>
          </div>
        </Reveal>
      </section>
      ...
      <ArticleCta column="constructions" />
