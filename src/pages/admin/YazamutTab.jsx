...
const aiPrompt = (a) => [
  a.coverAlt || a.title,
  a.category,
  'Israeli real estate, editorial magazine cover photography, high end architecture, modern, clean composition, natural light, premium, photorealistic, ultra detailed, no text, no logos, no watermark',
].filter(Boolean).join('. ')

/* ============================================================
   YazamutTab — ניהול כתבות "טור יזמות נדל"ן".
   כל כתבה בטאב נפרד: עריכת טקסט (ניסוח מחדש), קטגוריה, תגיות,
   החלפת תמונה (ריק = כריכה מעוצבת), ארכיון ומחיקה.
   נשמר בהגדרה אחת ('yazamut_articles') כמערך JSON; ממוזג עם
   כתבות הקוד (seed) לפי slug — עריכה גוברת על ה-seed.
   ============================================================ */

const CATS = ['התחדשות עירונית', 'מימון ויזמות', 'רגולציה ותכנון', 'קרקע ומכרזים', 'מגמות שוק']
...
export default function YazamutTab() {
  ...
  useEffect(() => {
    fetchSettings()
      .then((s) => {
        const merged = mergeArticles(s.yazamut_articles)
  ...
  const persist = async (next) => {
    ...
    setSaving(true)
    try {
      await setSetting('yazamut_articles', JSON.stringify(next))
    ...
    <div className="yzt">
      <div className="yzt__bar">
        <p className="yzt__intro">
          ניהול כתבות "טור יזמות נדל״ן" שמופיעות ב-<code>/yazamut-nadlan</code>. בחרו כתבה בטאב,
        ...
        <section className="yzt-edit" key={active.slug}>
          ...
          <div className="yzt-grid2">
            ...
            <label className="yzt-field"><span>זמן קריאה</span>
              <input dir="rtl" value={active.readingTime || ''} onChange={(e) => setField(active.slug, 'readingTime', e.target.value)} onBlur={commit} placeholder="4 דקות קריאה" />
            </label>
            ...
            <label className="yzt-field"><span>תגיות (מופרדות בפסיק)</span>
              <input dir="rtl" value={(active.tags || []).join(', ')} onChange={(e) => setField(active.slug, 'tags', e.target.value.split(',').map((t) => t.trim()).filter(Boolean))} onBlur={commit} placeholder="התחדשות, מימון, רגולציה" />
            </label>
          </div>

          <label className="yzt-field"><span>תקציר (excerpt)</span>
            <textarea dir="rtl" rows={2} value={active.excerpt || ''} onChange={(e) => setField(active.slug, 'excerpt', e.target.value)} onBlur={commit} placeholder="1-2 משפטים על הזווית של הכתבה" />
          </label>
          ...
          <div className="yzt-field">
            ...
            <div className="yzt-ai-row">
              <AiImageButton promptText={aiPrompt(active)} folder="yazamut" onImage={(url) => setAndSave(active.slug, 'cover', url)} />
